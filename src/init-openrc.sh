...
# Improved Exherbo installation script with OpenRC support
# Based on the original init.sh but configured for OpenRC init system
...
if [ $# -gt 1 ] || [[ $1 == "--help" ]]; then
print_header "Exherbo Setup with OpenRC - Exherbo Linux Installation Script"
echo "Usage:"
echo "  ./init-openrc.sh <device>    - Installs Exherbo Linux with OpenRC on the specified device"
echo "  ./init-openrc.sh             - Installs Exherbo Linux with OpenRC on /dev/sda by default"
echo ""
echo "Options:"
echo "  <device>    - The target device for Exherbo Linux installation"
echo "                Example: ./init-openrc.sh /dev/nvme0n1"
echo ""
echo "Description:"
echo "  Exherbo Setup is a simple script to automate the installation of Exherbo Linux."
echo "  This version is configured to use OpenRC as the init system instead of systemd."
...
format_disk() {
    print_status "Disk wiped successfully"
    echo
    print_status "Exherbo Linux with OpenRC will be installed on ${DISK}"
    echo
...
clear
print_header "Exherbo Linux Installation Script with OpenRC"
echo
...
clear
print_header "Exherbo Linux Installation with OpenRC"
echo
...
# Let's chroot!
cp "${SCRIPT_DIR}"/chrooted-openrc.sh /mnt/exherbo
if [ -f "${SCRIPT_DIR}"/params ]; then
    cp "${SCRIPT_DIR}"/params /mnt/exherbo
fi

print_status "Starting system configuration in chroot environment with OpenRC..."
env -i TERM="${TERM}" SHELL=/bin/bash HOME="${HOME}" "$(which chroot)" /mnt/exherbo /bin/bash chrooted-openrc.sh "${SYSTEM_TYPE}" "${DISK}"

# Cleanup and final steps
print_status "Cleaning up temporary files..."
rm /mnt/exherbo/chrooted-openrc.sh
...
echo
echo "The Exherbo Linux system with OpenRC has been installed and configured."
echo "System details:"
...
echo
echo "You can now:"
echo "1. Reboot to start using Exherbo Linux with OpenRC"
